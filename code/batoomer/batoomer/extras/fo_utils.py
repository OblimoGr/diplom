...
def fetch_followers(node, API, verbose, count=1000):
    """
    Given a node returns a DataFrame containing the names and descriptions of <count> followers. If the node is
    ...
    """
    my_print(f'Fetching followers of: {node}.', verbose)
    # Fetch friend IDs
    follower_ids = []
    try:
        ...
        my_print(f'Not authorized to fetch followers for node: {node}. Returning an empty DataFrame.', verbose)
    ...
    if not follower_ids:
        my_print(f'Node {node} has zero followers. Returning an empty DataFrame.', verbose)
        return pd.DataFrame(columns=['name', 'description'])

    # Calculate Iteration Required, to iterate per 100 ids
    if (int(len(follower_ids)) % 100) == 0:
        it_num = int(len(follower_ids) / 100)
    else:
        it_num = (int(len(follower_ids) / 100) + 1)

    # Transform IDs to User Objects
    users = list()
    try:
        for i in range(it_num):
            users.append(API.lookup_users(follower_ids[100 * i: 100 * (1 + i)]))
    except Exception as err:
        raise Exception(f'An unknown Error has occurred.\n{err}')
...
def calculate_parliament_followers(nodes, API, verbose, twitter_credentials):
    """
    Given a list of nodes. Extracts 1000 followers, classifies them, and calculates the number of followers, that are
    parliament members.
    ...
    my_print(f'\nCalculating number of followers that are parliament members. This will take some time.', verbose)
    for node in tqdm(nodes, leave=False, desc='Calculating followers parliament member count.'):
        # Get Required Data
        data = fetch_followers(node=node, API=API, verbose=0, count=1000)
